...
% set most of the variables to loop over here.
% actually pressure_l ravaries depeding
pressure_h_range = 2.0e5:10e4:10.0e5;
pressure_l_range = pressure_h_range;
m_dot_working_range = (1:10);
T_working_init_range = (9:1:39) + 273;

max_length = 1e3;

low_p = zeros(1,max_length);
high_p = low_p ;
P_out = low_p ;
P_in = low_p ;
T_pump = low_p ;
...
for pressure_h = pressure_h_range
...
%       cannot determine phase with given info
        ...
        for m_dot_working = m_dot_working_range
%             disp(['flow rate ' num2str(m_dot_working) ' kg/s'])
        for T_working_init = T_working_init_range
