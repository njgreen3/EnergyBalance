...
we = repmat(we',[1 500]);
wrm = 2/poles*wr*(60/(2*pi)); %rotor speed (rpm)
% wrm = 1/pole_pairs*wr*(60/(2*pi)); %rotor speed (rpm)
s = (we - wr)./we; %Slip


% Thevenin voltage & impedance
Vth = 1i*(Xm*2*pi./we)./(Rs + 1i*(Xs*2*pi./we) + 1i*(Xm*2*pi./we))*Vline_rated/sqrt(3);
...
Te = 3*(poles/2)*abs(Vth).^2*Rr./(s.*we)./((Rth + Rr./s).^2 + (Xth + Xr).^2);
Pe = Te.*wr*(2/poles);
